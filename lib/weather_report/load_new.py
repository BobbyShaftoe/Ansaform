...
def formatter(return_type=None):
    ...
    :param return_type: decorated function
    :return: func return
    """
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if return_type == 'string':
                return_data = str(result)
            elif return_type == 'json_string':
                return_data = json.dumps(result, indent=2)
            elif return_type == 'json_dict':
                return_data = json.loads(result)
            elif return_type == 'dataframe':
                return_data = pd.DataFrame(result)
            else:
                return_data = result
            return return_data
        return wrapper
...
if __name__ == "__main__":
    ...
    func_args = opt_parser.parse_args()[0]
