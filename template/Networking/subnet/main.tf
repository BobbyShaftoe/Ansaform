...
data "aws_vpc" "selected" {
  id = var.testing_vpc_id
}

# Create external facing subnets in us-east-1a AZ
resource "aws_subnet" "testing_ext_subnet_0" {
  count = 1
...
resource "aws_subnet" "testing_ext_subnet_1" {
  vpc_id = data.aws_vpc.selected.id
...
resource "aws_subnet" "testing_ext_subnet_2" {
  vpc_id = data.aws_vpc.selected.id
...
resource "aws_subnet" "testing_int_subnet_3" {
  vpc_id = data.aws_vpc.selected.id
  cidr_block = var.subnet_cidr[3]
  availability_zone = var.az[0]
  map_public_ip_on_launch = false
  tags = {
    Name = var.networking_tags[1]
  }
}

# Create internal facing subnets in us-east-1b AZ
resource "aws_subnet" "testing_int_subnet_4" {
  vpc_id = data.aws_vpc.selected.id
...
resource "aws_subnet" "testing_int_subnet_5" {
  vpc_id = data.aws_vpc.selected.id
